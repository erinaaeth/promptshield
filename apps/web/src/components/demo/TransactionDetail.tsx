...
function DataRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-start justify-between py-2.5 border-b border-border last:border-0">
      <span className="min-w-[96px] text-[11px] font-mono text-text-muted">{label}</span>
      <span
        className={`ml-4 max-w-[170px] break-words text-right text-[12px] font-mono ${
  ...
  return (
    <div className={`bg-surface border border-border rounded-2xl shadow-card flex flex-col h-full transition-all duration-300 ${processing ? "opacity-85" : "panel-float-in"}`}>
      ...
      <div className="px-4 py-3 border-b border-border">
        ...
        <h3 className="text-[15px] font-semibold text-text-primary">Generated Transaction</h3>
      </div>

      <div className="flex-1 flex flex-col gap-4.5 p-4 overflow-y-auto">
        {/* Risk indicator */}
        <div
          className={`rounded-xl border px-3.5 py-2.5 transition-all duration-300 ${risk.bg} ${risk.border} ${processing ? "processing-sheen bg-[length:220%_100%]" : ""}`}
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-text-muted">
              {phase === "ai" || phase === "draft" ? "Analysis State" : processing ? "Evaluation" : "Risk Level"}
            </span>
            <span className={`text-[12px] font-bold tracking-wide ${phase === "ai" || phase === "draft" || (processing && !policyReady) ? "text-warning" : risk.color}`}>
        ...
        <div>
          <div className="flex items-center justify-between mb-2 gap-3">
            ...
            <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] ${phaseChip.color}`}>
              {phaseChip.label}
            </span>
          </div>
          <div className="bg-surface-2 border border-border rounded-xl p-3 space-y-2">
            {analysisSummary.slice(0, 3).map((item, index) => (
              <div
                key={`${item}-${index}`}
                className={`flex items-start gap-2 text-[12px] leading-[1.58] transition-all duration-300 ${
        ...
        <div className="rounded-xl border border-border bg-surface-2 p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className={`text-[14px] font-bold ${riskScore >= 80 ? "text-danger" : riskScore >= 40 ? "text-warning" : "text-accent"}`}>
              {riskScore}%
            </span>
          </div>
        ...
        <div>
          ...
          <div className="bg-surface-2 border border-border rounded-xl px-3.5 py-0.5">
        ...
        <div className="rounded-xl border border-border bg-surface-2 p-3">
          ...
          <div className="mb-2.5 rounded-xl border border-border bg-surface px-3 py-2.5">
          ...
          <p className="mt-1 text-[11px] leading-[1.58] text-text-muted">
            {!walletConnected
